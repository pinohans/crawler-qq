...
{
	troycrawler::log::info(std::to_string(dwID) + WS2U8(L"开始注入"));

	TCHAR szModule[MAX_PATH];
	GetFullPathName(sModule, MAX_PATH, szModule, NULL);
	troycrawler::log::info(WS2U8(L"获得模块路径") + WS2U8(szModule));
	

	HANDLE hProcess = OpenProcess(PROCESS_ALL_ACCESS, FALSE, dwID);//打开进程
	if (!hProcess) {
		troycrawler::log::error(WS2U8(L"打开进程失败"));
		//CloseHandle(hToken);
		return FALSE;
	}
	...
	if (!pAddr || !WriteProcessMemory(hProcess, pAddr, szModule, cByte, NULL))//写入dll地址
	{
		...
		troycrawler::log::error(WS2U8(L"写入dll地址失败") + std::to_string(e));
	...
	if (!pfnStartAddr)
	{
		troycrawler::log::error(WS2U8(L"LoadLibraryW地址获取失败"));
		VirtualFreeEx(hProcess, pAddr, cByte, MEM_RELEASE);
		CloseHandle(hProcess);
		return FALSE;
	}
	...
	if (!hRemoteThread)
	{
		troycrawler::log::error(WS2U8(L"远程线程创建失败"));
		VirtualFreeEx(hProcess, pAddr, cByte, MEM_RELEASE);
		CloseHandle(hProcess);
		return FALSE;
	}
				...
				if (wcscmp(pe32.szExeFile, L"QQ.exe") == 0)
				{
					...
					if (bNeedInject)
					{
						if (LoadLib(TEXT("crawler.dll"), dwPid))
							troycrawler::log::info(std::to_string(dwPid) + WS2U8(L"进程注入成功"));
						else
							troycrawler::log::info(std::to_string(dwPid) + WS2U8(L"进程注入失败"));
					}
				END_PRESENT_PROCESS:
					CloseHandle(hModuleSnap);
				}
